...
export default function SystemEstimates({ profile }) {
  ...
  async function loadEstimates(orgId) {
    ...
    const [estimatesRes, usersRes] = await Promise.all([
      supabase
        ...
        .select(`
          ...
          profit, profit_pct, sent_at, sent_count, last_sent_to, paid_at, estimating_technician_id, approval_status, approved_at, spawned_job_id, converted_to_job_id, is_archived, reference_job_id, estimate_type, bills_to_customer_id, property_id
        `)
        .eq('org_id', orgId)
        .eq('kind', 'estimate')
        .is('deleted_at', null)
    ...
    let estimates = (estimatesRes.data || []).filter((e) => e.estimate_type === 'system')
    ...
    let custById = {}
    if (custIds.length) {
      const { data: custs } = await supabase.from('customers').select('id, display_name, primary_phone').in('id', custIds)
      custById = Object.fromEntries((custs || []).map((cu) => [cu.id, cu]))
    }
    const propIds = [...new Set(estimates.map((e) => e.property_id).filter(Boolean))]
    let propById = {}
    if (propIds.length) {
      const { data: props } = await supabase.from('properties').select('id, street_address, unit, city').in('id', propIds)
      propById = Object.fromEntries((props || []).map((pr) => [pr.id, pr]))
    }
    estimates = estimates.map((e) => ({
      ...e,
      customer: e.bills_to_customer_id ? custById[e.bills_to_customer_id] || null : null,
      property: e.property_id ? propById[e.property_id] || null : null,
  ...
  function customerName(est) {
    return est.customer?.display_name || 'Unknown'
  }

  function customerMobile(est) {
    return formatPhone(est.customer?.primary_phone) || ''
  }
    ...
    <div>
      <div className="page-header-bar">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h2>System Estimates</h2>
          <span className="badge">{estimates.length.toLocaleString()} total</span>
        </div>
        <button className="auth-button" style={{ width: 'auto', padding: '8px 18px' }} onClick={() => navigate('/new-system-estimate')}>+ New System Estimate</button>
      </div>
            ...
            {sorted.map((est, rowIdx) => {
              ...
              <div key={est.id} style={{ display: 'contents' }}>
                <div ref={isHighlight ? highlightRef : undefined} className="grid-cell grid-actions" style={actionsCellStyle(rowBg)}>
                  <Link to={'/system-estimate-p/' + est.id} className="logout-button" style={{ textDecoration: 'none', display: 'inline-block' }}>
                    Edit
                  </Link>
