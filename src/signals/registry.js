...
export const REGISTRY = [
  ...
  { key: 'parts-ready-schedule', station: 'dispatch', hub: 'start', name: 'Parts in — schedule return', tone: 'amber', href: '/calendar', cta: 'Open calendar', audience: 'office',
    line: (n) => `${n} staged visit${n === 1 ? '' : 's'} unblocked — the part arrived`,
    count: headCount('jobs', (q) => q.eq('parts_ready', true).eq('date_pending', true).is('deleted_at', null).neq('status', 'completed')) },
  ...
  { key: 'payments-to-confirm', station: 'work', hub: 'start', name: 'Payments to confirm', tone: 'amber', href: '/payments-to-confirm', cta: 'Open payments queue', audience: 'office',
    line: (n) => `${n} invoice${n === 1 ? '' : 's'} with a chosen method to confirm or mark paid`,
    count: headCount('invoices', (q) => q.eq('kind', 'invoice').not('is_filter_order', 'is', true).is('deleted_at', null).not('payment_method', 'is', null).is('paid_at', null)) },
  { key: 'estimates-to-convert', station: 'work', hub: 'start', name: 'Estimates to convert', tone: 'amber', href: '/estimates', cta: 'Open estimates', audience: 'office',
    ...
    count: headCount('invoices', (q) => q.eq('kind', 'estimate').ilike('approval_status', 'approved').is('spawned_job_id', null).is('converted_to_job_id', null).is('deleted_at', null).eq('is_archived', false).or('estimate_type.is.null,estimate_type.neq.system')) },
