        ...
        <div className="grid-table" style={{ gridTemplateColumns: '1fr 1fr 1.5fr 1fr 1fr' }}>
          ...
          {sorted.map((inv) => (
            <Link key={inv.id} to={'/invoice/' + inv.job_id} style={{ display: 'contents', textDecoration: 'none', color: 'inherit' }}>
              ...
              {visibleColumns.includes('customer') && <div className="grid-cell">{customerName(inv)}</div>}
              {visibleColumns.includes('amount_due') && <div className="grid-cell">${inv.amount_due?.toFixed(2)}</div>}
              {visibleColumns.includes('status') && (
