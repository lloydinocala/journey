...
export default function CallConsole({ profile }) {
  ...
  useEffect(() => {
    setPurpose(''); setCallerName(''); setLoggedMsg(''); setNeedsCb(false); setRouteTo(''); setFollowUp(false)
    ;(async () => {
      let q = null
  ...
  async function logCall() {
    ...
    await supabase.from('call_logs').insert({
      ...
      route_to: routeTo || null, follow_up: followUp,
      taken_by: profile?.user_id || null, taken_by_name: profile?.full_name || null,
    })
    setLogging(false); setLoggedMsg('Call logged.'); setPurpose(''); setCallerName(''); setNeedsCb(false); setRouteTo(''); setFollowUp(false)
            ...
            <div className="section-card" style={{ padding: 18 }}>
              ...
              <div style={{ marginTop: 14, borderTop: '1px solid var(--border)', paddingTop: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <select value={routeTo} onChange={(e) => setRouteTo(e.target.value)} title="Route this message to" style={{ padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 8 }}>
                    <option value="">Route to…</option>
                    <option value="Dispatch">Dispatch</option>
                    <option value="Jobs Mgmt">Jobs Mgmt</option>
                  ...
                  <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12.5, color: 'var(--mist)', cursor: 'pointer', whiteSpace: 'nowrap' }}><input type="checkbox" checked={followUp} onChange={(e) => setFollowUp(e.target.checked)} /> Follow-up</label>
